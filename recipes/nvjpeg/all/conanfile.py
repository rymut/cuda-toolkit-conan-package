...
class NvJpegConan(ConanFile):
    name = "nvjpeg"
    description = "nvJPEG - NVidia GPU-accelerated JPEG decoder, encoder and transcoder"
    url = "https://github.com/conan-io/conan-center-index"
    homepage = "https://developer.nvidia.com/cuda-downloads"
    license = "Nvidia CUDA Toolkit EULA"
    topics = ("nvjpeg", "cuda", "nvidia")
    ...
    def requirements(self):
        if not self.options.shared:
            ...
            self.requires("cudart/[>={self.release_version}")
    ...
    def package_info(self):
        ...
        self.cpp_info.set_property("cmake_target_aliases", ["CUDA::nvjpeg", "CUDA::nvjpeg_static"])
        if not self.options.shared:
            self.cpp_info.requires = ["cublibos::cublibos", "cudart::cudart_deps"]
            self.cpp_info.libs = ["nvjpeg"]
        else:
            self.cpp_info.libs = ["nvjpeg_static"]
